import { ArrowUpRight, LogIn, X } from 'lucide-react'
import { type SyntheticEvent, useEffect, useState } from 'react'
import { dateLabel, getJson, sendJson, SESSION_ID } from '../lib/api'
import type { Comment, Section, Story } from '../types'
import { StoryImage } from './StoryImage'

type ReaderModalProps = { story: Story; section: Section; authToken: string; authName: string; onClose: () => void; onOpenAuth: () => void }

export function ReaderModal({ story, section, authToken, authName, onClose, onOpenAuth }: ReaderModalProps) {
  ...
  useEffect(() => {
    const endpoint = section === 'news' ? `/news/${story.id}/view` : `/blogs/${story.id}/view`
    sendJson(endpoint, { method: 'POST', body: JSON.stringify({ sessionId: SESSION_ID, duration: 1, scrolled: false }) }).catch(() => undefined)
    if (section === 'blogs') {
      const loadingTimer = window.setTimeout(() => setLoading(true), 0)
      getJson<Comment[]>(`/comments/blog/${story.id}`).then(setComments).catch(() => setError('Comments could not be loaded.')).finally(() => setLoading(false))
  ...
  const addComment = async (event: SyntheticEvent<HTMLFormElement>) => {
    ...
    if (!commentText.trim() || !authToken) return
    try {
